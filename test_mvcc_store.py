...
class TestVersionVisibility:
    ...
    def test_concurrent_read_modify_write(self):
        """Test concurrent read-modify-write operations showing last-write-wins"""
        store = MVCCStore()

        # T1: Insert initial values
        txn1 = store.begin()
        store.put(txn1, "key1", 100)
        store.put(txn1, "key2", 100)
        store.put(txn1, "key3", 100)
        store.commit(txn1)

        # T2 and T3: Start concurrent transactions
        txn2 = store.begin()
        txn3 = store.begin()

        # Both read initial values
        txn2_key1_val = store.get(txn2, "key1")  # 100
        txn3_key1_val = store.get(txn3, "key1")  # 100
        txn3_key3_val = store.get(txn3, "key3")  # 100
        txn2_key2_val = store.get(txn2, "key2")  # 100

        # Both calculate updates based on initial reads
        store.put(txn2, "key1", txn2_key1_val + txn2_key2_val)  # 200
        store.put(txn2, "key2", 0)
        store.put(txn3, "key1", txn3_key1_val + txn3_key3_val)  # 200
        store.put(txn3, "key3", 0)

        # Commit in order - last write wins
        store.commit(txn2)
        store.commit(txn3)

        txn = store.begin()
        assert store.get(txn, "key1") == 300  # T3's value wins
